...
##############  THIS CONTROLLER WORK WHEN WE CLICK ON CATALOG PDF BUTTON AND IT DOWNLOAD THE PDF #############
class PortalMyProductDetailsPdf(http.Controller):
    ...
    def portal_my_product_details_pdf(self, **kw):
        print("=====button clicked PDF download=====")
        ...
        pdf_content, _ = report_service._render_qweb_pdf(
            'wbl_website_pdf_catalog.action_report_all_product_template_pdf', [], values
        )
...
#         allowed_category_ids = setting_product_category_ids_command[0][
#             2] if setting_product_category_ids_command else []
#         # Fetch all products which is published.
#         products = request.env['product.template'].sudo().search([
#             ('website_published', '=', True),
#             ('categ_id', 'in', allowed_category_ids),
#         ])
#         print("======", products)
#
#         # Prepare values for the template
...
#         # Render the PDF
#         report_service = request.env['ir.actions.report']
#         pdf_content, _ = report_service._render_qweb_pdf('wbl_website_pdf_catalog.action_report_all_product_template_pdf',
#                                                          [], values)
#
#         # Create a response to download the PDF
#         response = request.make_response(pdf_content, headers=[
...
##############  THIS CONTROLLER WORK WHEN WE CLICK ON CLICK TO VIEW AND IT VIEW THE PDF #############
class PortalMyProductDetailsPdfView(http.Controller):
    @http.route(['/product/details/category/pdf/view'], methods=["GET"], auth='public', website=True)
    def portal_my_product_details_pdf_view(self, **kw):
        print("=====button clicked VIEW=====")
        settings = request.env['res.config.settings'].sudo().get_values()
        # print("====settings===", settings)
        ...
        pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
            'wbl_website_pdf_catalog.action_report_all_product_template_view', [], values)
        ...
        # # Render the PDF
        # report_service = request.env['ir.actions.report']
        # pdf_content, _ = report_service._render_qweb_pdf('wbl_website_pdf_catalog.action_report_all_product_template_view',
        #                                                  [], values)
        #
        # # Create a response to download the PDF
        # response = request.make_response(pdf_content, headers=[
